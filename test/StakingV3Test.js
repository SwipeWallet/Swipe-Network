...
const SXPTOKEN = require("../build/SwipeToken")
...
describe('StakingV3 Tests', () => {
    ...
    beforeEach(async () => {
        sxpToken = await deployContract(tokenHolder, SXPTOKEN, [])
    ...
    describe('Upgrade', () => {
        beforeEach(async () => {
            const calldata = getCalldata('initialize', ['address', 'address', 'address'], [walletGuardian.address, sxpToken.address, walletRewardProvider.address])
        ...
        it('Reinitialize must revert', async () => {
            ...
            expect(await implementation._sxpTokenAddress()).to.be.equal(sxpToken.address)
        ...
        it('Storage is saved after upgrade', async () => {
            ...
            expect(await implementationOld._sxpTokenAddress()).to.be.equal(sxpToken.address)
    ...
    describe('Reward Pool', () => {
        ...
        it('Deposit SXP by wrong reward provider', async () => {
            ...
            await sxpToken.transfer(walletGuardian.address, amount)
            await sxpToken.connect(walletGuardian).approve(proxy.address, amount)
            const implementation = new ethers.Contract(proxy.address, STAKINGV3.interface, walletGuardian)
            const beforeBalance = await sxpToken.balanceOf(walletGuardian.address)
            expect(beforeBalance).to.be.equal(amount)
            await expect(implementation.depositRewardPool(amount)).to.be.reverted
            const afterBalance = await sxpToken.balanceOf(walletGuardian.address)
            expect(afterBalance).to.be.equal(amount)
        })

        it('Deposit SXP', async () => {
            const amount = '1000000000000000000000'
            await sxpToken.transfer(walletRewardProvider.address, amount)
            await sxpToken.connect(walletRewardProvider).approve(proxy.address, amount)
            const implementation = new ethers.Contract(proxy.address, STAKINGV3.interface, walletRewardProvider)
            ...
            const afterBalance = await sxpToken.balanceOf(proxy.address)
            expect(afterBalance).to.be.equal(amount)
        ...
        it('Withdraw SXP by wrong reward provider', async () => {
            ...
            await sxpToken.transfer(walletRewardProvider.address, depositAmount)
            await sxpToken.connect(walletRewardProvider).approve(proxy.address, depositAmount)
            ...
            const implementation = new ethers.Contract(proxy.address, STAKINGV3.interface, walletGuardian)
            const beforeBalance = await sxpToken.balanceOf(proxy.address)
        ...
        it('Withdraw SXP', async () => {
            ...
            const beforeRewardProviderBalance = await sxpToken.balanceOf(walletRewardProvider.address)
            ...
            const afterRewardProviderBalance = await sxpToken.balanceOf(walletRewardProvider.address)
    ...
    describe('Stake', () => {
        ...
        it('Stake SXP', async () => {
            const amount = '1000000000000000000000'
            await sxpToken.approve(proxy.address, amount)
            const beforeBalance = await sxpToken.balanceOf(proxy.address)
        ...
        it('Withdraw SXP by wrong staker', async () => {
            ...
            await sxpToken.approve(proxy.address, stakeAmount)
            ...
            const amount = '300000000000000000000'
            const implementation = new ethers.Contract(proxy.address, STAKINGV3.interface, walletGuardian)
            const beforeWithdrawerBalance = await sxpToken.balanceOf(walletGuardian.address)
        ...
        it('Withdraw SXP', async () => {
            ...
            const amount = '300000000000000000000'
            const remainAmount = '700000000000000000000'
            const beforeHolderBalance = await sxpToken.balanceOf(tokenHolder.address)
            ...
            const afterHolderBalance = await sxpToken.balanceOf(tokenHolder.address)
        ...
        it('Get prior staked amount', async () => {
            ...
            await sxpToken.approve(proxy.address, amountToStake)
            const implementation = new ethers.Contract(proxy.address, STAKINGV3.interface, tokenHolder)
